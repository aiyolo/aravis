...
static gboolean gstreamer_plugin_check(void)
{
    ...
    if (g_once_init_enter(&check_done))
    {
        ...
        static char *plugins[] = {"appsrc", "videoconvert", "videoflip"};
...
static double arv_viewer_value_to_log(double value, double min, double max)
{
    if (min >= max)
        return 1.0;
...
static double arv_viewer_value_from_log(double value, double min, double max)
{
    if (min <= 0.0 || max <= 0)
        return 0.0;
...
static void notification_dismiss_clicked_cb(GtkButton *dismiss, ArvViewer *viewer)
{
...
static void arv_viewer_show_notification(ArvViewer *viewer, const char *message, const char *details)
{
...
static void gst_buffer_release_cb(void *user_data)
{
...
static GstBuffer *arv_to_gst_buffer(ArvBuffer *arv_buffer, guint part_id, ArvStream *stream)
{
    ...
    return gst_buffer_new_wrapped_full(GST_MEMORY_FLAG_READONLY, data, size, 0, size, release_data, gst_buffer_release_cb);
}

static void new_buffer_cb(ArvStream *stream, ArvViewer *viewer)
{
    ArvBuffer *arv_buffer;
    gint n_input_buffers, n_output_buffers;
...
static void _apply_frame_rate(GtkEntry *entry, ArvViewer *viewer, gboolean grab_focus)
{
    char *text;
    double frame_rate;
...
static void frame_rate_entry_cb(GtkEntry *entry, ArvViewer *viewer)
{
    _apply_frame_rate(entry, viewer, TRUE);
}

static gboolean frame_rate_entry_focus_cb(GtkEntry *entry, GdkEventFocus *event, ArvViewer *viewer)
{
...
static void exposure_spin_cb(GtkSpinButton *spin_button, ArvViewer *viewer)
{
...
static void gain_spin_cb(GtkSpinButton *spin_button, ArvViewer *viewer)
{
...
static void black_level_spin_cb(GtkSpinButton *spin_button, ArvViewer *viewer)
{
...
static void exposure_scale_cb(GtkRange *range, ArvViewer *viewer)
{
...
static void gain_scale_cb(GtkRange *range, ArvViewer *viewer)
{
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(viewer->auto_gain_toggle), FALSE);
...
static void black_level_scale_cb(GtkRange *range, ArvViewer *viewer)
{
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(viewer->auto_black_level_toggle), FALSE);
...
static gboolean update_exposure_cb(void *data)
{
...
static void update_exposure_ui(ArvViewer *viewer, gboolean is_auto)
{
...
static void auto_exposure_cb(GtkToggleButton *toggle, ArvViewer *viewer)
{
    gboolean is_auto;
...
static gboolean update_gain_cb(void *data)
{
    ArvViewer *viewer = data;
    double gain;
...
static void update_gain_ui(ArvViewer *viewer, gboolean is_auto)
{
...
static void auto_gain_cb(GtkToggleButton *toggle, ArvViewer *viewer)
{
    gboolean is_auto;
...
static gboolean update_black_level_cb(void *data)
{
    ArvViewer *viewer = data;
    double black_level;
...
static void update_black_level_ui(ArvViewer *viewer, gboolean is_auto)
{
...
static void auto_black_level_cb(GtkToggleButton *toggle, ArvViewer *viewer)
{
    gboolean is_auto;
...
static void set_camera_widgets(ArvViewer *viewer)
{
    ...
    gtk_widget_set_sensitive(viewer->auto_gain_toggle, arv_camera_is_gain_auto_available(viewer->camera, NULL));
    gtk_widget_set_sensitive(viewer->auto_black_level_toggle, arv_camera_is_black_level_auto_available(viewer->camera, NULL));
    gtk_widget_set_sensitive(viewer->auto_exposure_toggle, arv_camera_is_exposure_auto_available(viewer->camera, NULL));
...
static gboolean _save_gst_sample_to_file(GstSample *sample, const char *path, const char *mime_type, GError **error)
{
    GstSample *converted;
    GstCaps *caps;
...
static void snapshot_cb(GtkButton *button, ArvViewer *viewer)
{
    ...
    filename = g_strdup_printf("%s-%s-%d-%d-%s-%s.raw", arv_camera_get_vendor_name(viewer->camera, NULL),
        arv_camera_get_device_serial_number(viewer->camera, NULL), width, height, pixel_format != NULL ? pixel_format : "Unknown",
        date_string);
    g_free(date_string);
    ...
    dialog = gtk_file_chooser_dialog_new("Save Snapshot", GTK_WINDOW(viewer->main_window), GTK_FILE_CHOOSER_ACTION_SAVE, "_Cancel",
    ...
    if (result == GTK_RESPONSE_ACCEPT)
    {
        ...
        if (!success)
            arv_viewer_show_notification(viewer, "Failed to save image to file", error != NULL ? error->message : NULL);
    }
...
static void update_transform(ArvViewer *viewer)
{
    static const gint methods[4][4] = {{0, 1, 2, 3}, {4, 6, 5, 7}, {5, 7, 4, 6}, {2, 3, 0, 1}};
...
static void rotate_cw_cb(GtkButton *button, ArvViewer *viewer)
{
...
static void flip_horizontal_cb(GtkToggleButton *toggle, ArvViewer *viewer)
{
...
static void flip_vertical_cb(GtkToggleButton *toggle, ArvViewer *viewer)
{
...
static gboolean update_status_bar_cb(void *data)
{
    ArvViewer *viewer = data;
    char *text;
    ...
    text = g_strdup_printf("%.1f fps (%.1f MB/s)", 1000.0 * (n_images - viewer->last_n_images) / elapsed_time_ms,
        ((n_bytes - viewer->last_n_bytes) / 1000.0) / elapsed_time_ms);
    gtk_label_set_label(GTK_LABEL(viewer->fps_label), text);
    g_free(text);

    text = g_strdup_printf("%" G_GUINT64_FORMAT " image%s / %" G_GUINT64_FORMAT " error%s", n_images, n_images > 0 ? "s" : "", n_errors,
        n_errors > 0 ? "s" : "");
...
static void update_camera_region(ArvViewer *viewer)
{
...
static void update_pixel_format(ArvViewer *viewer)
{
    ...
    for (i = 0; i < n_pixel_formats; i++)
    {
        ...
        {
            if (current_format < 0 || g_strcmp0(pixel_format_strings[i], pixel_format_string) == 0)
                current_format = i;
            n_valid_formats++;
            valid = TRUE;
        }

        gtk_list_store_set(list_store, &iter, 0, pixel_format_strings[i], 1, valid, -1);
    }
    g_free(pixel_formats);
    g_free(pixel_format_strings);
    gtk_widget_set_sensitive(viewer->pixel_format_combo, n_valid_formats > 0);
    gtk_widget_set_sensitive(
        viewer->video_mode_button, n_valid_formats > 0 && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(viewer->component_check)));

    gtk_combo_box_set_active(GTK_COMBO_BOX(viewer->pixel_format_combo), current_format >= 0 ? current_format : 0);

    gtk_widget_set_tooltip_text(GTK_WIDGET(viewer->pixel_format_combo), bayer_tooltip
...
static void camera_region_cb(GtkSpinButton *spin_button, ArvViewer *viewer)
{
...
static void camera_binning_cb(GtkSpinButton *spin_button, ArvViewer *viewer)
{
...
static void component_combo_cb(GtkComboBoxText *combo, ArvViewer *viewer)
{
    char *component;
    gboolean enabled;

    component = gtk_combo_box_text_get_active_text(combo);
    enabled = arv_camera_select_component(viewer->camera, component, ARV_COMPONENT_SELECTION_FLAGS_NONE, NULL, NULL);
    g_free(component);
...
static void component_toggled_cb(GtkToggleButton *button, ArvViewer *viewer)
{
    char *component;
    gboolean enable;
    ...
    arv_camera_select_component(
        viewer->camera, component, enable ? ARV_COMPONENT_SELECTION_FLAGS_ENABLE : ARV_COMPONENT_SELECTION_FLAGS_DISABLE, NULL, NULL);

    g_free(component);
}

static void pixel_format_combo_cb(GtkComboBoxText *combo, ArvViewer *viewer)
{
    char *pixel_format;
...
static void update_device_list_cb(GtkToolButton *button, ArvViewer *viewer)
{
    GtkListStore *list_store;
    GtkTreeIter iter;
    ...
    for (i = 0; i < n_devices; i++)
    {
        ...
        gtk_list_store_append(list_store, &iter);
        gtk_list_store_set(list_store, &iter, 0, arv_get_device_id(i), 1, protocol->str, 2, arv_get_device_vendor(i), 3,
            arv_get_device_model(i), 4, arv_get_device_serial_nbr(i), -1);
...
static void remove_widget(GtkWidget *widget, gpointer data)
{
...
static gboolean start_video(ArvViewer *viewer)
{
    ...
    if (component != NULL)
    {
        arv_camera_select_component(viewer->camera, component, ARV_COMPONENT_SELECTION_FLAGS_NONE, &viewer->component_id, NULL);
        g_free(component);
    }
    ...
    if (ARV_IS_GV_STREAM(viewer->stream))
    {
        if (viewer->auto_socket_buffer)
            g_object_set(viewer->stream, "socket-buffer", ARV_GV_STREAM_SOCKET_BUFFER_AUTO, "socket-buffer-size", 0, NULL);
        if (!viewer->packet_resend)
            g_object_set(viewer->stream, "packet-resend", ARV_GV_STREAM_PACKET_RESEND_NEVER, NULL);
        g_object_set(viewer->stream, "initial-packet-timeout", (unsigned)viewer->initial_packet_timeout * 1000, "packet-timeout",
            (unsigned)viewer->packet_timeout * 1000, "frame-retention", (unsigned)viewer->frame_retention * 1000, NULL);
    }
...
gst_caps_set_simple(caps, "width", G_TYPE_INT, width, "height", G_TYPE_INT, height, "framerate", GST_TYPE_FRACTION, 0, 1, NULL);
...
static gboolean select_camera_list_mode(gpointer user_data)
{
    ArvViewer *viewer = user_data;
...
static void control_lost_cb(ArvCamera *camera, ArvViewer *viewer)
{
    g_main_context_invoke(NULL, select_camera_list_mode, viewer);
}

static void stop_camera(ArvViewer *viewer)
{
    gtk_revealer_set_reveal_child(GTK_REVEALER(viewer->camera_parameters), FALSE);
    gtk_widget_set_sensitive(viewer->camera_parameters, FALSE);
    gtk_widget_set_sensitive(viewer->video_mode_button, FALSE);
...
static void set_sensitive(GtkCellLayout *cell_layout, GtkCellRenderer *cell, GtkTreeModel *tree_model, GtkTreeIter *iter, gpointer data)
{
    gboolean valid;
...
static gboolean start_camera(ArvViewer *viewer, const char *camera_id)
{
    GtkTreeIter iter;
    GtkListStore *list_store;
    ...
    arv_device_set_register_cache_policy(arv_camera_get_device(viewer->camera), viewer->register_cache_policy);
    arv_device_set_range_check_policy(arv_camera_get_device(viewer->camera), viewer->range_check_policy);
    ...
    if (arv_camera_is_component_available(viewer->camera, NULL))
    {
        ...
        {
            gtk_list_store_append(list_store, &iter);
            gtk_list_store_set(list_store, &iter, 0, components[i], -1);

            if (first_enabled_component < 0 &&
                arv_camera_select_component(viewer->camera, components[i], ARV_COMPONENT_SELECTION_FLAGS_NONE, NULL, NULL))
...
static void camera_selection_changed_cb(GtkTreeSelection *selection, ArvViewer *viewer)
{
    GtkTreeIter iter;
...
static void select_mode(ArvViewer *viewer, ArvViewerMode mode)
{
    ...
    case ARV_VIEWER_MODE_VIDEO:
        ...
        arv_camera_get_region(viewer->camera, &x, &y, &width, &height, NULL);
        subtitle = g_strdup_printf("%s %dx%d@%d,%d %s", arv_camera_get_model_name(viewer->camera, NULL), width, height, x, y,
...
static void switch_to_camera_list_cb(GtkToolButton *button, ArvViewer *viewer)
{
    select_mode(viewer, ARV_VIEWER_MODE_CAMERA_LIST);
}

static void switch_to_video_mode_cb(GtkToolButton *button, ArvViewer *viewer)
{
    select_mode(viewer, ARV_VIEWER_MODE_VIDEO);
}

static void arv_viewer_quit_cb(GtkApplicationWindow *window, ArvViewer *viewer)
{
    stop_camera(viewer);
    g_application_quit(G_APPLICATION(viewer));
}

static void video_frame_realize_cb(GtkWidget *widget, ArvViewer *viewer)
{
#ifdef GDK_WINDOWING_X11
...
static void activate(GApplication *application)
{
    ...
    g_signal_connect(viewer->notification_dismiss, "clicked", G_CALLBACK(notification_dismiss_clicked_cb), viewer);
    ...
    viewer->rotate_cw_clicked = g_signal_connect(viewer->rotate_cw_button, "clicked", G_CALLBACK(rotate_cw_cb), viewer);
    viewer->flip_horizontal_clicked = g_signal_connect(viewer->flip_horizontal_toggle, "clicked", G_CALLBACK(flip_horizontal_cb), viewer);
    viewer->flip_vertical_clicked = g_signal_connect(viewer->flip_vertical_toggle, "clicked", G_CALLBACK(flip_vertical_cb), viewer);
    ...
    viewer->camera_selected = g_signal_connect(
        gtk_tree_view_get_selection(GTK_TREE_VIEW(viewer->camera_tree)), "changed", G_CALLBACK(camera_selection_changed_cb), viewer);
    viewer->exposure_spin_changed = g_signal_connect(viewer->exposure_spin_button, "value-changed", G_CALLBACK(exposure_spin_cb), viewer);
    viewer->exposure_hscale_changed = g_signal_connect(viewer->exposure_hscale, "value-changed", G_CALLBACK(exposure_scale_cb), viewer);
    viewer->auto_exposure_clicked = g_signal_connect(viewer->auto_exposure_toggle, "clicked", G_CALLBACK(auto_exposure_cb), viewer);
    viewer->gain_spin_changed = g_signal_connect(viewer->gain_spin_button, "value-changed", G_CALLBACK(gain_spin_cb), viewer);
    viewer->gain_hscale_changed = g_signal_connect(viewer->gain_hscale, "value-changed", G_CALLBACK(gain_scale_cb), viewer);
    viewer->auto_gain_clicked = g_signal_connect(viewer->auto_gain_toggle, "clicked", G_CALLBACK(auto_gain_cb), viewer);
    viewer->black_level_spin_changed =
        g_signal_connect(viewer->black_level_spin_button, "value-changed", G_CALLBACK(black_level_spin_cb), viewer);
    viewer->black_level_hscale_changed =
        g_signal_connect(viewer->black_level_hscale, "value-changed", G_CALLBACK(black_level_scale_cb), viewer);
    viewer->auto_black_level_clicked =
        g_signal_connect(viewer->auto_black_level_toggle, "clicked", G_CALLBACK(auto_black_level_cb), viewer);
    viewer->component_changed = g_signal_connect(viewer->component_combo, "changed", G_CALLBACK(component_combo_cb), viewer);
    viewer->component_toggled = g_signal_connect(viewer->component_check, "toggled", G_CALLBACK(component_toggled_cb), viewer);
    viewer->pixel_format_changed = g_signal_connect(viewer->pixel_format_combo, "changed", G_CALLBACK(pixel_format_combo_cb), viewer);
    viewer->camera_x_changed = g_signal_connect(viewer->camera_x, "value-changed", G_CALLBACK(camera_region_cb), viewer);
    viewer->camera_y_changed = g_signal_connect(viewer->camera_y, "value-changed", G_CALLBACK(camera_region_cb), viewer);
    viewer->camera_width_changed = g_signal_connect(viewer->camera_width, "value-changed", G_CALLBACK(camera_region_cb), viewer);
    viewer->camera_height_changed = g_signal_connect(viewer->camera_height, "value-changed", G_CALLBACK(camera_region_cb), viewer);
    viewer->camera_binning_x_changed = g_signal_connect(viewer->camera_binning_x, "value-changed", G_CALLBACK(camera_binning_cb), viewer);
    viewer->camera_binning_y_changed = g_signal_connect(viewer->camera_binning_y, "value-changed", G_CALLBACK(camera_binning_cb), viewer);
...
static void viewer_shutdown(GApplication *application)
{
...
ArvViewer *arv_viewer_new(void)
{
    ...
    arv_viewer = g_object_new(
        arv_viewer_get_type(), "application-id", "org.aravis.Aravis", "flags", G_APPLICATION_NON_UNIQUE, "inactivity-timeout", 30000, NULL);

    return arv_viewer;
}

static void arv_viewer_init(ArvViewer *viewer)
{
...
static void arv_viewer_class_init(ArvViewerClass *class)
{
