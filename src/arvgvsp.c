...
static ArvGvspPacket *arv_gvsp_packet_new(
    ArvGvspContentType content_type, guint16 frame_id, guint32 packet_id, size_t data_size, void *buffer, size_t *buffer_size)
{
    ArvGvspPacket *packet;
    ArvGvspHeader *header;
    size_t packet_size;

	// ArvGvspPacket + ArvGvspHeader + data
	// TODO: 为什么是这个大小
    ...
    header->packet_infos = g_htonl((packet_id & ARV_GVSP_PACKET_ID_MASK) |
                                   ((content_type << ARV_GVSP_PACKET_INFOS_CONTENT_TYPE_POS) & ARV_GVSP_PACKET_INFOS_CONTENT_TYPE_MASK));

    return packet;
}

// leader包
ArvGvspPacket *arv_gvsp_packet_new_image_leader(guint16 frame_id, guint32 packet_id, guint64 timestamp, ArvPixelFormat pixel_format,
    guint32 width, guint32 height, guint32 x_offset, guint32 y_offset, guint32 x_padding, guint32 y_padding, void *buffer,
    size_t *buffer_size)
{
    ArvGvspPacket *packet;

 // 构造gvsp包
    packet = arv_gvsp_packet_new(ARV_GVSP_CONTENT_TYPE_LEADER, frame_id, packet_id, sizeof(ArvGvspImageLeader), buffer, buffer_size);
...
ArvGvspPacket *arv_gvsp_packet_new_data_trailer(guint16 frame_id, guint32 packet_id, void *buffer, size_t *buffer_size)
{
    ...
    packet = arv_gvsp_packet_new(ARV_GVSP_CONTENT_TYPE_TRAILER, frame_id, packet_id, sizeof(ArvGvspTrailer), buffer, buffer_size);
...
// 将data处大小为size的内容写入packet
// buffer_size修改成packet_size=header+data_size 
// 如果buffer不为空，buffer的地址赋值给packet
ArvGvspPacket *arv_gvsp_packet_new_payload(guint16 frame_id, guint32 packet_id, size_t size, void *data, void *buffer, size_t *buffer_size)
{
    ArvGvspPacket *packet;

    packet = arv_gvsp_packet_new(ARV_GVSP_CONTENT_TYPE_PAYLOAD, frame_id, packet_id, size, buffer, buffer_size);
...
static const char *arv_enum_to_string(GType type, guint enum_value)
{
...
static const char *arv_gvsp_packet_type_to_string(ArvGvspPacketType value)
{
    return arv_enum_to_string(ARV_TYPE_GVSP_PACKET_TYPE, value);
}

static const char *arv_gvsp_content_type_to_string(ArvGvspContentType value)
{
...
char *arv_gvsp_packet_to_string(const ArvGvspPacket *packet, size_t packet_size)
{
    ...
    case ARV_GVSP_CONTENT_TYPE_LEADER:
        ...
        if (payload_type == ARV_BUFFER_PAYLOAD_TYPE_IMAGE || payload_type == ARV_BUFFER_PAYLOAD_TYPE_EXTENDED_CHUNK_DATA)
        {
            ArvPixelFormat pixel_format;
            guint32 width, height, x_offset, y_offset, x_padding, y_padding;

            if (arv_gvsp_leader_packet_get_image_infos(
                    packet, &pixel_format, &width, &height, &x_offset, &y_offset, &x_padding, &y_padding))
            {
