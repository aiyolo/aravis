...
enum
{
    ...
    ARV_GV_FAKE_CAMERA_INPUT_SOCKET_GLOBAL_DISCOVERY, // 全局发现
    ARV_GV_FAKE_CAMERA_INPUT_SOCKET_SUBNET_DISCOVERY, // 子网发现
...
} ArvGvFakeCameraPrivate; // fakegv相机私有数据
...
static gboolean _g_inet_socket_address_is_equal(GInetSocketAddress *a, GInetSocketAddress *b)
{
    ...
    return g_inet_address_equal(g_inet_socket_address_get_address(a), g_inet_socket_address_get_address(b));
}

// 处理控制包
static gboolean _handle_control_packet(
    ArvGvFakeCamera *gv_fake_camera, GSocket *socket, GSocketAddress *remote_address, ArvGvcpPacket *packet, size_t size)
{
    ...
    if (gv_fake_camera->priv->controller_address != NULL)
    {
        ...
        if (elapsed_ms > arv_fake_camera_get_heartbeat_timeout(gv_fake_camera->priv->camera))
        {
            ...
            write_access = _g_inet_socket_address_is_equal(
                G_INET_SOCKET_ADDRESS(remote_address), G_INET_SOCKET_ADDRESS(gv_fake_camera->priv->controller_address));
    }
    else
        write_access = TRUE;
    ...
    case ARV_GVCP_COMMAND_DISCOVERY_CMD:
        ...
        // 准备ack数据
        arv_fake_camera_read_memory(gv_fake_camera->priv->camera, 0, ARV_GVBS_DISCOVERY_DATA_SIZE, &ack_packet->data);
        break;
    case ARV_GVCP_COMMAND_READ_MEMORY_CMD:
        arv_gvcp_packet_get_read_memory_cmd_infos(packet, &block_address, &block_size);
        arv_info_device("[GvFakeCamera::handle_control_packet] Read memory command %d (%d)", block_address, block_size);
        ack_packet = arv_gvcp_packet_new_read_memory_ack(block_address, block_size, packet_id, &ack_packet_size);
        arv_fake_camera_read_memory(
            gv_fake_camera->priv->camera, block_address, block_size, arv_gvcp_packet_get_read_memory_ack_data(ack_packet));
        break;
    case ARV_GVCP_COMMAND_WRITE_MEMORY_CMD:
        ...
        {
            arv_warning_device(
                "[GvFakeCamera::handle_control_packet] Ignore Write memory command %d (%d) not controller", block_address, block_size);
            break;
        }

        arv_info_device("[GvFakeCamera::handle_control_packet] Write memory command %d (%d)", block_address, block_size);
        arv_fake_camera_write_memory(
            gv_fake_camera->priv->camera, block_address, block_size, arv_gvcp_packet_get_write_memory_cmd_data(packet));
        ack_packet = arv_gvcp_packet_new_write_memory_ack(block_address, packet_id, &ack_packet_size);
        break;
    case ARV_GVCP_COMMAND_READ_REGISTER_CMD:
        ...
        arv_info_device("[GvFakeCamera::handle_control_packet] Read register command %d -> %d", register_address, register_value);
        ack_packet = arv_gvcp_packet_new_read_register_ack(register_value, packet_id, &ack_packet_size);
    ...
    case ARV_GVCP_COMMAND_WRITE_REGISTER_CMD:
        ...
        arv_info_device("[GvFakeCamera::handle_control_packet] Write register command %d -> %d", register_address, register_value);
        ack_packet = arv_gvcp_packet_new_write_register_ack(1, packet_id, &ack_packet_size);
        break;
    default:
...
static void *_thread(void *user_data)
{
    ...
    do
    {
            ...
            if (n_events > 0)
            {
                ...
                for (i = 0; i < ARV_GV_FAKE_CAMERA_N_INPUT_SOCKETS; i++)
                {
                    ...
                    if (G_IS_SOCKET(socket))
                    {
						...
						// 读取数据,对方的地址存为 remote_address
                        count = g_socket_receive_message(socket, &remote_address, &input_vector, 1, NULL, NULL, NULL, NULL, NULL);
                        if (count > 0)
                        {
							// 处理控制包
                            if (_handle_control_packet(gv_fake_camera, socket, remote_address, input_vector.buffer, count))
            ...
            arv_fake_camera_get_acquisition_status(gv_fake_camera->priv->camera) != 0)
        {
            if (stream_address == NULL)
            {
                ...
                arv_info_stream_thread("[GvFakeCamera::thread] Start stream to %s (%d)", inet_address_string,
                    ...
                    arv_fake_camera_check_and_acknowledge_software_trigger(gv_fake_camera->priv->camera)))
            {
				...
				// 构造 packet_buffer
                arv_gvsp_packet_new_image_leader(image_buffer->priv->frame_id, block_id, arv_buffer_get_timestamp(image_buffer),
                    arv_buffer_get_image_pixel_format(image_buffer), arv_buffer_get_image_width(image_buffer),
                    arv_buffer_get_image_height(image_buffer), arv_buffer_get_image_x(image_buffer), arv_buffer_get_image_y(image_buffer),
                    0, 0, packet_buffer, &packet_size);
                // 发送 packet_buffer 到 stream address
                if (g_random_double() >= gv_fake_camera->priv->gvsp_lost_packet_ratio)
                    g_socket_send_to(gv_fake_camera->priv->gvsp_socket, stream_address, packet_buffer, packet_size, NULL, &error);
                else
                    arv_info_stream_thread("Drop GVSP leader packet frame: %" G_GUINT64_FORMAT, image_buffer->priv->frame_id);

                if (error != NULL)
                {
                    arv_warning_stream_thread("[GvFakeCamera::thread] Failed to send leader for frame %" G_GUINT64_FORMAT ": %s",
                ...
                while (offset < payload)
                {
                    ...
                    data_size = MIN(gv_packet_size - ARV_GVSP_PACKET_PROTOCOL_OVERHEAD(FALSE), payload - offset);

                    packet_size = ARV_GV_FAKE_CAMERA_BUFFER_SIZE;
					// 数据包的大小现在为data_size，内容为buffer
                    arv_gvsp_packet_new_payload(image_buffer->priv->frame_id, block_id, data_size,
                        ((char *)image_buffer->priv->data) + offset, packet_buffer, &packet_size);
					...
					// 循环发送数据包
                        ...
                        arv_info_stream_thread(
                            "Drop GVSP data packet frame:%" G_GUINT64_FORMAT ", block:%u", image_buffer->priv->frame_id, block_id);
				...
				// trailer数据包
                packet_size = ARV_GV_FAKE_CAMERA_BUFFER_SIZE;
                arv_gvsp_packet_new_data_trailer(image_buffer->priv->frame_id, block_id, packet_buffer, &packet_size);

                if (g_random_double() >= gv_fake_camera->priv->gvsp_lost_packet_ratio)
                    g_socket_send_to(gv_fake_camera->priv->gvsp_socket, stream_address, packet_buffer, packet_size, NULL, &error);
                else
                    arv_info_stream_thread("Drop GVSP trailer packet frame: %" G_GUINT64_FORMAT, image_buffer->priv->frame_id);

                if (error != NULL)
                {
                    arv_info_stream_thread("[GvFakeCamera::thread] Failed to send trailer for frame %" G_GUINT64_FORMAT ": %s",
                        image_buffer->priv->frame_id, error->message);
                    g_clear_error(&error);
                }
...
static gboolean _create_and_bind_input_socket(
    GSocket **socket_out, const char *socket_name, GInetAddress *inet_address, unsigned int port, gboolean allow_reuse, gboolean blocking)
{
    GSocket *socket;
...
// device
static gboolean arv_gv_fake_camera_start(ArvGvFakeCamera *gv_fake_camera)
{
    ...
    {
        arv_warning_device(
            "[GvFakeCamera::start] No network interface with address or name '%s' found.", gv_fake_camera->priv->interface_name);
        return FALSE;
    }

    socket_address = g_socket_address_new_from_native(arv_network_interface_get_addr(iface), sizeof(struct sockaddr));
	// 网络地址 127.0.0.1
	...
	// 绑定 gvsp socket GVSP address = 127.0.0.1
    _create_and_bind_input_socket(&gv_fake_camera->priv->gvsp_socket, "GVSP", gvcp_inet_address, 0, FALSE, TRUE);
	// 绑定gvcp socekt 在同一个地址上，但是端口不同, 3956
	//  GVCP address = 127.0.0.1:3956
    _create_and_bind_input_socket(
        &gv_fake_camera->priv->input_sockets[ARV_GV_FAKE_CAMERA_INPUT_SOCKET_GVCP], "GVCP", gvcp_inet_address, ARV_GVCP_PORT, FALSE, FALSE);

    inet_address = g_inet_address_new_from_string("255.255.255.255");
	// 如果网络地址不是 255.255.255.255
    if (!g_inet_address_equal(gvcp_inet_address, inet_address))
	// 全局发现 udp socket, 绑定在 255.255.255.255:3956
    // Global discovery address = 255.255.255.255:3956
        _create_and_bind_input_socket(&gv_fake_camera->priv->input_sockets[ARV_GV_FAKE_CAMERA_INPUT_SOCKET_GLOBAL_DISCOVERY],
            "Global discovery", inet_address, ARV_GVCP_PORT, TRUE, FALSE);
    g_clear_object(&inet_address);
    g_clear_object(&socket_address);

	// 从接口获取 广播地址
    socket_address = g_socket_address_new_from_native(arv_network_interface_get_broadaddr(iface), sizeof(struct sockaddr));
    inet_address = g_object_ref(g_inet_socket_address_get_address(G_INET_SOCKET_ADDRESS(socket_address)));
    if (!g_inet_address_equal(gvcp_inet_address, inet_address))
	// 绑定子网发现地址 无
        _create_and_bind_input_socket(&gv_fake_camera->priv->input_sockets[ARV_GV_FAKE_CAMERA_INPUT_SOCKET_SUBNET_DISCOVERY],
...
static void arv_gv_fake_camera_stop(ArvGvFakeCamera *gv_fake_camera)
{
    unsigned int i;
...
{
    return g_object_new(ARV_TYPE_GV_FAKE_CAMERA, "interface-name",
        interface_name != NULL ? interface_name : ARV_GV_FAKE_CAMERA_DEFAULT_INTERFACE, "serial-number",
        serial_number != NULL ? serial_number : ARV_GV_FAKE_CAMERA_DEFAULT_SERIAL_NUMBER, "genicam-filename", genicam_filename, NULL);
}
...
static void arv_gv_fake_camera_init(ArvGvFakeCamera *gv_fake_camera)
{
    gv_fake_camera->priv = arv_gv_fake_camera_get_instance_private(gv_fake_camera);
}

static void _constructed(GObject *gobject)
{
...
static void arv_gv_fake_camera_class_init(ArvGvFakeCameraClass *this_class)
{
    ...
    g_object_class_install_property(object_class, PROP_INTERFACE_NAME,
        g_param_spec_string("interface-name", "Interface name", "Interface name", ARV_GV_FAKE_CAMERA_DEFAULT_INTERFACE,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
    g_object_class_install_property(object_class, PROP_SERIAL_NUMBER,
        g_param_spec_string("serial-number", "Serial number", "Serial number", ARV_GV_FAKE_CAMERA_DEFAULT_SERIAL_NUMBER,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
    g_object_class_install_property(object_class, PROP_GENICAM_FILENAME,
        g_param_spec_string("genicam-filename", "Genicam filename", "Genicam filename", NULL,
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
    g_object_class_install_property(object_class, PROP_GVSP_LOST_PACKET_RATIO,
        g_param_spec_double("gvsp-lost-ratio", "GVSP lost packet ratio", "GVSP lost packet ratio", 0.0, 1.0, 0.0,
