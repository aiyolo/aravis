...
static const char *arv_get_fake_camera_genicam_filename(void)
{
...
static guint32 _get_register(ArvFakeCamera *camera, guint32 address)
{
    guint32 value;
...
static void arv_fake_camera_diagonal_ramp(
...
void arv_fake_camera_fill_buffer(ArvFakeCamera *camera, ArvBuffer *buffer, guint32 *packet_size)
{
    ...
    camera->priv->fill_pattern_callback(buffer, camera->priv->fill_pattern_data, exposure_time_us, gain, pixel_format);
	...
	// TODO: packet大小
    if (packet_size != NULL)
        *packet_size = (_get_register(camera, ARV_GVBS_STREAM_CHANNEL_0_PACKET_SIZE_OFFSET) >> ARV_GVBS_STREAM_CHANNEL_0_PACKET_SIZE_POS) &
...
void arv_fake_camera_set_inet_address(ArvFakeCamera *camera, GInetAddress *address)
{
    ...
    arv_fake_camera_write_memory(camera, ARV_GVBS_CURRENT_IP_ADDRESS_OFFSET, g_inet_address_get_native_size(address), (char *)bytes);
}
...
GSocketAddress *arv_fake_camera_get_stream_address(ArvFakeCamera *camera)
{
    ...
    stream_socket_address = g_inet_socket_address_new(inet_address, _get_register(camera, ARV_GVBS_STREAM_CHANNEL_0_PORT_OFFSET));
...
gboolean arv_fake_camera_check_and_acknowledge_software_trigger(ArvFakeCamera *camera)
{
    g_return_val_if_fail(ARV_IS_FAKE_CAMERA(camera), FALSE);
...
gboolean arv_fake_camera_is_in_free_running_mode(ArvFakeCamera *camera)
{
    g_return_val_if_fail(ARV_IS_FAKE_CAMERA(camera), FALSE);
...
gboolean arv_fake_camera_is_in_software_trigger_mode(ArvFakeCamera *camera)
{
    g_return_val_if_fail(ARV_IS_FAKE_CAMERA(camera), FALSE);
...
ArvFakeCamera *arv_fake_camera_new_full(const char *serial_number, const char *genicam_filename)
{
    ...
    if (filename)
    {
        ...
        {
            g_critical("Failed to load genicam file '%s': %s", filename, error != NULL ? error->message : "Unknown reason");
            g_clear_error(&error);
    ...
    {
        GBytes *bytes = g_resources_lookup_data("/org/aravis/arv-fake-camera.xml", G_RESOURCE_LOOKUP_FLAGS_NONE, &error);
    ...
    xml_url =
        g_strdup_printf("Local:arv-fake-camera.xml;%x;%x", ARV_FAKE_CAMERA_MEMORY_SIZE, (unsigned int)fake_camera->priv->genicam_xml_size);
    strcpy(((char *)memory) + ARV_GVBS_XML_URL_0_OFFSET, xml_url);
    g_free(xml_url);

    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_SENSOR_WIDTH, ARV_FAKE_CAMERA_SENSOR_WIDTH);
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_SENSOR_HEIGHT, ARV_FAKE_CAMERA_SENSOR_HEIGHT);
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_WIDTH, ARV_FAKE_CAMERA_WIDTH_DEFAULT);
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_HEIGHT, ARV_FAKE_CAMERA_HEIGHT_DEFAULT);
    ...
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_BINNING_HORIZONTAL, ARV_FAKE_CAMERA_BINNING_HORIZONTAL_DEFAULT);
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_BINNING_VERTICAL, ARV_FAKE_CAMERA_BINNING_HORIZONTAL_DEFAULT);
    ...
    arv_fake_camera_write_register(
        fake_camera, ARV_FAKE_CAMERA_REGISTER_ACQUISITION_FRAME_PERIOD_US, 1000000.0 / ARV_FAKE_CAMERA_ACQUISITION_FRAME_RATE_DEFAULT);
    arv_fake_camera_write_register(fake_camera, ARV_FAKE_CAMERA_REGISTER_EXPOSURE_TIME_US, ARV_FAKE_CAMERA_EXPOSURE_TIME_US_DEFAULT);
...
static void arv_fake_camera_init(ArvFakeCamera *fake_camera)
{
...
static void arv_fake_camera_finalize(GObject *object)
{
...
static void arv_fake_camera_class_init(ArvFakeCameraClass *fake_camera_class)
{
...
static void arv_fake_camera_destructor(void)
{
