...
static void *arv_fake_stream_thread(void *data)
{
    ...
    while (!g_atomic_int_get(&thread_data->cancel))
    {
        ...
        if (buffer != NULL)
        {
            ...
            if (thread_data->callback != NULL)
                thread_data->callback(thread_data->callback_data, ARV_STREAM_CALLBACK_TYPE_START_BUFFER, NULL);
            ...
            if (thread_data->callback != NULL)
                thread_data->callback(thread_data->callback_data, ARV_STREAM_CALLBACK_TYPE_BUFFER_DONE, buffer);
        }
        else
...
static void arv_fake_stream_start_thread(ArvStream *stream)
{
...
static void arv_fake_stream_stop_thread(ArvStream *stream)
{
    ArvFakeStream *fake_stream = ARV_FAKE_STREAM(stream);
    ArvFakeStreamPrivate *priv = arv_fake_stream_get_instance_private(fake_stream);
    ArvFakeStreamThreadData *thread_data;
...
ArvStream *arv_fake_stream_new(
    ArvFakeDevice *device, ArvStreamCallback callback, void *callback_data, GDestroyNotify destroy, GError **error)
{
    return g_initable_new(ARV_TYPE_FAKE_STREAM, NULL, error, "device", device, "callback", callback, "callback-data", callback_data,
        "destroy-notify", destroy, NULL);
}

static void arv_fake_stream_constructed(GObject *object)
{
    ...
    g_object_get(object, "device", &fake_device, "callback", &thread_data->callback, "callback-data", &thread_data->callback_data, NULL);

    thread_data->fake_camera = arv_fake_device_get_fake_camera(fake_device);

    thread_data->cancel = FALSE;

    arv_stream_declare_info(ARV_STREAM(fake_stream), "n_completed_buffers", G_TYPE_UINT64, &thread_data->n_completed_buffers);
    arv_stream_declare_info(ARV_STREAM(fake_stream), "n_failures", G_TYPE_UINT64, &thread_data->n_failures);
    arv_stream_declare_info(ARV_STREAM(fake_stream), "n_underruns", G_TYPE_UINT64, &thread_data->n_underruns);
    arv_stream_declare_info(ARV_STREAM(fake_stream), "n_transferred_bytes", G_TYPE_UINT64, &thread_data->n_transferred_bytes);
    arv_stream_declare_info(ARV_STREAM(fake_stream), "n_ignored_bytes", G_TYPE_UINT64, &thread_data->n_ignored_bytes);
...
static void arv_fake_stream_init(ArvFakeStream *fake_stream) {}

static void arv_fake_stream_finalize(GObject *object)
{
    ArvFakeStream *fake_stream = ARV_FAKE_STREAM(object);
    ArvFakeStreamPrivate *priv = arv_fake_stream_get_instance_private(fake_stream);
...
static void arv_fake_stream_class_init(ArvFakeStreamClass *fake_stream_class)
{
