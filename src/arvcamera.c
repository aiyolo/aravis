...
G_DEFINE_TYPE_WITH_CODE(
    ArvCamera, arv_camera, G_TYPE_OBJECT, G_ADD_PRIVATE(ArvCamera) G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, arv_camera_initable_iface_init))
...
ArvStream *arv_camera_create_stream(ArvCamera *camera, ArvStreamCallback callback, gpointer user_data, GError **error)
{
...
ArvStream *arv_camera_create_stream_full(
    ArvCamera *camera, ArvStreamCallback callback, gpointer user_data, GDestroyNotify destroy, GError **error)
{
...
gboolean arv_camera_is_region_offset_available(ArvCamera *camera, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
    GError *local_error = NULL;
...
void arv_camera_set_acquisition_mode(ArvCamera *camera, ArvAcquisitionMode acquisition_mode, GError **error)
{
    ...
    case ARV_CAMERA_VENDOR_BASLER:
        ...
        if (local_error == NULL)
            arv_camera_set_float(
                camera, priv->has_acquisition_frame_rate ? "AcquisitionFrameRate" : "AcquisitionFrameRateAbs", frame_rate, &local_error);
        break;
...
double arv_camera_get_frame_rate(ArvCamera *camera, GError **error)
{
    ...
    case ARV_CAMERA_VENDOR_IMPERX:
    case ARV_CAMERA_VENDOR_UNKNOWN:
        return arv_camera_get_float(camera, priv->has_acquisition_frame_rate ? "AcquisitionFrameRate" : "AcquisitionFrameRateAbs", error);
    }
...
void arv_camera_get_frame_rate_bounds(ArvCamera *camera, double *min, double *max, GError **error)
{
    ...
    case ARV_CAMERA_VENDOR_UNKNOWN:
        arv_camera_get_float_bounds(
            camera, priv->has_acquisition_frame_rate ? "AcquisitionFrameRate" : "AcquisitionFrameRateAbs", min, max, error);
        break;
    }
}
 ...
 * Since: 0.8.26
 */
void arv_camera_set_frame_rate_enable(ArvCamera *camera, gboolean enable, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
    GError *local_error = NULL;
...
void arv_camera_set_trigger(ArvCamera *camera, const char *source, GError **error)
{
    ...
    if (has_trigger_selector == TRUE)
    {
        ...
        triggers = arv_camera_dup_available_enumerations_as_strings(camera, "TriggerSelector", &n_triggers, &local_error);
    ...
    if (local_error == NULL)
    {
        ...
        if (local_error == NULL && arv_camera_is_enumeration_entry_available(camera, "TriggerActivation", "RisingEdge", NULL))
...
gboolean arv_camera_is_software_trigger_supported(ArvCamera *camera, GError **error)
{
...
void arv_camera_set_exposure_time(ArvCamera *camera, double exposure_time_us, GError **error)
{
    ...
    default:
        arv_camera_set_float(camera, priv->has_exposure_time ? "ExposureTime" : "ExposureTimeAbs", exposure_time_us, &local_error);
        break;
    }
...
double arv_camera_get_exposure_time(ArvCamera *camera, GError **error)
{
    ...
    default:
        return arv_camera_get_float(camera, priv->has_exposure_time ? "ExposureTime" : "ExposureTimeAbs", error);
    }
}
...
void arv_camera_get_exposure_time_bounds(ArvCamera *camera, double *min, double *max, GError **error)
{
    ...
    {
    case ARV_CAMERA_SERIES_BASLER_SCOUT:
        arv_camera_get_float_bounds(camera, priv->has_exposure_time ? "ExposureTime" : "ExposureTimeBaseAbs", min, max, error);
        break;
    case ARV_CAMERA_SERIES_BASLER_ACE:
    ...
    case ARV_CAMERA_SERIES_RICOH:
        ...
        break;
    default:
        arv_camera_get_float_bounds(camera, priv->has_exposure_time ? "ExposureTime" : "ExposureTimeAbs", min, max, error);
        break;
    }
}
...
void arv_camera_set_exposure_mode(ArvCamera *camera, ArvExposureMode acquisition_mode, GError **error)
{
...
void arv_camera_select_gain(ArvCamera *camera, const char *selector, GError **error)
{
...
const char **arv_camera_dup_available_gains(ArvCamera *camera, guint *n_selectors, GError **error)
{
    g_return_val_if_fail(ARV_IS_CAMERA(camera), NULL);
...
gboolean arv_camera_is_black_level_available(ArvCamera *camera, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_select_black_level(ArvCamera *camera, const char *selector, GError **error)
{
...
const char **arv_camera_dup_available_black_levels(ArvCamera *camera, guint *n_selectors, GError **error)
{
    g_return_val_if_fail(ARV_IS_CAMERA(camera), NULL);
...
void arv_camera_set_black_level_auto(ArvCamera *camera, ArvAuto auto_mode, GError **error)
{
...
ArvAuto arv_camera_get_black_level_auto(ArvCamera *camera, GError **error)
{
    g_return_val_if_fail(ARV_IS_CAMERA(camera), ARV_AUTO_OFF);
...
gboolean arv_camera_is_component_available(ArvCamera *camera, GError **error)
{
...
const char **arv_camera_dup_available_components(ArvCamera *camera, guint *n_components, GError **error)
{
    g_return_val_if_fail(ARV_IS_CAMERA(camera), NULL);
...
gboolean arv_camera_select_component(
    ArvCamera *camera, const char *component, ArvComponentSelectionFlags flags, guint *component_id, GError **error)
{
    GError *local_error = NULL;
    ...
    if (flags == ARV_COMPONENT_SELECTION_FLAGS_EXCLUSIVE_ENABLE || flags == ARV_COMPONENT_SELECTION_FLAGS_ENABLE_ALL)
    {
        ...
        for (i = 0; i < n_components && local_error == NULL; i++)
        {
            ...
            if (local_error == NULL)
                arv_camera_set_boolean(camera, "ComponentEnable", flags == ARV_COMPONENT_SELECTION_FLAGS_ENABLE_ALL, &local_error);
        }
    }

    if (local_error == NULL)
        arv_camera_set_string(camera, "ComponentSelector", component, &local_error);
    if (local_error == NULL && flags != ARV_COMPONENT_SELECTION_FLAGS_NONE && flags != ARV_COMPONENT_SELECTION_FLAGS_ENABLE_ALL)
        arv_camera_set_boolean(camera, "ComponentEnable", flags == ARV_COMPONENT_SELECTION_FLAGS_DISABLE ? FALSE : TRUE, &local_error);
...
void arv_camera_select_and_enable_component(ArvCamera *camera, const char *component, gboolean disable_others, GError **error)
{
    arv_camera_select_component(camera, component,
        disable_others ? ARV_COMPONENT_SELECTION_FLAGS_EXCLUSIVE_ENABLE : ARV_COMPONENT_SELECTION_FLAGS_ENABLE, NULL, error);
}
...
gboolean arv_camera_is_frame_rate_available(ArvCamera *camera, GError **error)
{
    ...
    case ARV_CAMERA_VENDOR_IMPERX:
    case ARV_CAMERA_VENDOR_UNKNOWN:
        return arv_camera_is_feature_available(
            camera, priv->has_acquisition_frame_rate ? "AcquisitionFrameRate" : "AcquisitionFrameRateAbs", error);
    }
...
gboolean arv_camera_is_exposure_time_available(ArvCamera *camera, GError **error)
{
    ...
    default:
        return arv_camera_is_feature_available(camera, priv->has_exposure_time ? "ExposureTime" : "ExposureTimeAbs", error);
    }
}
...
gboolean arv_camera_is_black_level_auto_available(ArvCamera *camera, GError **error)
{
...
gint64 *arv_camera_dup_available_enumerations(ArvCamera *camera, const char *feature, guint *n_values, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
const char **arv_camera_dup_available_enumerations_as_strings(ArvCamera *camera, const char *feature, guint *n_values, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
const char **arv_camera_dup_available_enumerations_as_display_names(ArvCamera *camera, const char *feature, guint *n_values, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
gboolean arv_camera_is_enumeration_entry_available(ArvCamera *camera, const char *feature, const char *entry, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
gboolean arv_camera_is_feature_implemented(ArvCamera *camera, const char *feature, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_set_access_check_policy(ArvCamera *camera, ArvAccessCheckPolicy policy)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
gint arv_camera_gv_get_n_network_interfaces(ArvCamera *camera, GError **error)
{
...
void arv_camera_gv_set_multipart(ArvCamera *camera, gboolean enable, GError **error)
{
    g_return_if_fail(arv_camera_is_gv_device(camera));
...
gboolean arv_camera_gv_get_multipart(ArvCamera *camera, GError **error)
{
...
gboolean arv_camera_gv_is_multipart_supported(ArvCamera *camera, GError **error)
{
    GError *local_error = NULL;
    ...
    if (local_error != NULL)
    {
        if (local_error->domain == ARV_DEVICE_ERROR && (local_error->code == ARV_DEVICE_ERROR_PROTOCOL_ERROR_INVALID_ADDRESS ||
...
void arv_camera_gv_set_packet_delay(ArvCamera *camera, gint64 delay_ns, GError **error)
{
    ...
    if (tick_frequency <= 0)
    {
        if (!arv_camera_is_feature_available(camera, "GevSCPD", NULL))
            g_set_error(error, ARV_DEVICE_ERROR, ARV_DEVICE_ERROR_FEATURE_NOT_FOUND, "GevSCPD not not found");
        else
            g_set_error(error, ARV_DEVICE_ERROR, ARV_DEVICE_ERROR_PROTOCOL_ERROR, "Device returned an invalid timestamp tick frequency");
        return;
    }
...
void arv_camera_gv_set_packet_size_adjustment(ArvCamera *camera, ArvGvPacketSizeAdjustment adjustment)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_gv_get_persistent_ip(ArvCamera *camera, GInetAddress **ip, GInetAddressMask **mask, GInetAddress **gateway, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_gv_set_persistent_ip_from_string(ArvCamera *camera, const char *ip, const char *mask, const char *gateway, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_gv_set_persistent_ip(ArvCamera *camera, GInetAddress *ip, GInetAddressMask *mask, GInetAddress *gateway, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
ArvGvIpConfigurationMode arv_camera_gv_get_ip_configuration_mode(ArvCamera *camera, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_gv_set_ip_configuration_mode(ArvCamera *camera, ArvGvIpConfigurationMode mode, GError **error)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
...
void arv_camera_uv_set_usb_mode(ArvCamera *camera, ArvUvUsbMode usb_mode)
{
#if ARAVIS_HAS_USB
    ArvCameraPrivate *priv = arv_camera_get_instance_private(camera);
#endif
...
void arv_camera_set_chunks(ArvCamera *camera, const char *chunk_list, GError **error)
{
    ...
    if (local_error == NULL)
    {
        ...
        for (j = 0; chunks[j] != NULL && local_error == NULL; j++)
        {
            ...
            {
                g_set_error(
                    &local_error, ARV_DEVICE_ERROR, ARV_DEVICE_ERROR_FEATURE_NOT_FOUND, "%s not found in available chunks", chunks[j]);
            }
        }
...
static void arv_camera_init(ArvCamera *camera) {}

static void arv_camera_finalize(GObject *object)
{
...
static void arv_camera_constructed(GObject *object)
{
    ...
    if (!ARV_IS_DEVICE(priv->device))
    {
        ...
        {
            if (priv->name != NULL)
                error = g_error_new(ARV_DEVICE_ERROR, ARV_DEVICE_ERROR_NOT_FOUND, "Device '%s' not found", priv->name);
            else
                error = g_error_new(ARV_DEVICE_ERROR, ARV_DEVICE_ERROR_NOT_FOUND, "No supported device found");
        }
    ...
    priv->has_acquisition_frame_rate = ARV_IS_GC_FLOAT(arv_device_get_feature(priv->device, "AcquisitionFrameRate"));
    priv->has_acquisition_frame_rate_auto = ARV_IS_GC_STRING(arv_device_get_feature(priv->device, "AcquisitionFrameRateAuto"));
    priv->has_acquisition_frame_rate_enabled = ARV_IS_GC_BOOLEAN(arv_device_get_feature(priv->device, "AcquisitionFrameRateEnabled"));
...
static void arv_camera_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(ARV_CAMERA(object));
...
static void arv_camera_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    ArvCameraPrivate *priv = arv_camera_get_instance_private(ARV_CAMERA(object));
...
static void arv_camera_class_init(ArvCameraClass *camera_class)
{
    ...
    g_object_class_install_property(object_class, PROP_CAMERA_NAME,
        g_param_spec_string("name", "Camera name", "The camera name", NULL, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
    ...
    g_object_class_install_property(object_class, PROP_CAMERA_DEVICE,
        g_param_spec_object("device", "Device", "The device associated with this camera", ARV_TYPE_DEVICE,
            G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}

static gboolean arv_camera_initable_init(GInitable *initable, GCancellable *cancellable, GError **error)
{
...
// 将 arv_camera_initable_init 函数注册为 GInitableIface 接口的 init 函数，以便在创建实现该接口的对象时，可以通过调用 init 函数来进行初始化操作。
static void arv_camera_initable_iface_init(GInitableIface *iface)
{
