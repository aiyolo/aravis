...
ArvGvcpPacket *arv_gvcp_packet_new_write_memory_cmd(
    guint32 address, guint32 size, const char *buffer, guint16 packet_id, size_t *packet_size)
{
    ArvGvcpPacket *packet;
    guint32 n_address = g_htonl(address);
    guint32 actual_size;
...
ArvGvcpPacket *arv_gvcp_packet_new_discovery_cmd(gboolean allow_broadcat_discovery_ack, size_t *packet_size)
{
    ArvGvcpPacket *packet;

    g_return_val_if_fail(packet_size != NULL, NULL);

    *packet_size = sizeof(ArvGvcpHeader); // no data

    packet = g_malloc(*packet_size);

    packet->header.packet_type = ARV_GVCP_PACKET_TYPE_CMD;
    packet->header.packet_flags =
        ARV_GVCP_CMD_PACKET_FLAGS_ACK_REQUIRED | (allow_broadcat_discovery_ack ? ARV_GVCP_DISCOVERY_PACKET_FLAGS_ALLOW_BROADCAST_ACK : 0);
...
char *arv_gvcp_packet_flags_to_string_new(ArvGvcpCommand command, guint8 flags)
{
    ...
    {
        if ((1 << i) & flags)
            g_string_append_printf(string, "%s%s", string->len > 0 ? " " : "", arv_enum_to_string(ARV_TYPE_GVCP_CMD_PACKET_FLAGS, 1 << i));
    }
        ...
        for (i = 0; i < 8; i++)
        {
            if ((1 << i) & flags)
                g_string_append_printf(
                    string, "%s%s", string->len > 0 ? " " : "", arv_enum_to_string(ARV_TYPE_GVCP_DISCOVERY_PACKET_FLAGS, 1 << i));
        }
        break;
    case ARV_GVCP_COMMAND_PACKET_RESEND_CMD:
        for (i = 0; i < 8; i++)
        {
            if ((1 << i) & flags)
                g_string_append_printf(
                    string, "%s%s", string->len > 0 ? " " : "", arv_enum_to_string(ARV_TYPE_GVCP_EVENT_PACKET_FLAGS, 1 << i));
        }
        break;
    default:
        break;
...
char *arv_gvcp_packet_to_string(const ArvGvcpPacket *packet)
{
    ...
    case ARV_GVCP_COMMAND_DISCOVERY_ACK:
        ...
        g_string_append_printf(string, "address      = %d.%d.%d.%d\n", data[ARV_GVBS_CURRENT_IP_ADDRESS_OFFSET] & 0xff,
            data[ARV_GVBS_CURRENT_IP_ADDRESS_OFFSET + 1] & 0xff, data[ARV_GVBS_CURRENT_IP_ADDRESS_OFFSET + 2] & 0xff,
